...
console.log('JSON:', histStr);

db.init().then(async () => {
  try {
    const result = await db.run(
      `UPDATE crm_clients SET entreprise = ?, secteur = ?, contact = ?, fonction = ?, email = ?, tel = ?, source = ?, stade = ?, besoin = ?, commentaires = ?, linkedin = ?, prochaine_action = ?, historique = ?::json WHERE id = ?`,
