...
router.get('/', (req, res) => {
  ...
  else          { sql += ` AND status = 'published'`; } // public = publiés seulement
  if (category) { sql += ` AND category = ?`; params.push(category); }
  sql += ` ORDER BY created_at DESC`;
  const publications = db.all(sql, params);
  res.json({ publications, total: publications.length });
});

// GET /api/publications/all — admin (tous statuts)
router.get('/all', auth, (req, res) => {
  const publications = db.all(`SELECT * FROM publications ORDER BY created_at DESC`);
  res.json({ publications, total: publications.length });
});

// GET /api/publications/:id — public
router.get('/:id', (req, res) => {
  const pub = db.get(`SELECT * FROM publications WHERE id = ?`, [req.params.id]);
...
router.post('/', auth, upload.single('image'), (req, res) => {
  ...
  const pub = db.get(`SELECT * FROM publications WHERE id = ?`, [id]);
...
router.put('/:id', auth, upload.single('image'), (req, res) => {
  const { title, category, status, excerpt, content } = req.body;
  const existing = db.get(`SELECT * FROM publications WHERE id = ?`, [req.params.id]);
  ...
  db.run(
    `UPDATE publications SET title=?, category=?, status=?, excerpt=?, content=?, image_url=?, published_at=?, updated_at=datetime('now')
     WHERE id=?`,
    [title || existing.title, category || existing.category, status || existing.status, excerpt || null, content || null, image_url, published_at, req.params.id]
  );
  res.json(db.get(`SELECT * FROM publications WHERE id = ?`, [req.params.id]));
});
...
router.patch('/:id/status', auth, (req, res) => {
  ...
  db.run(`UPDATE publications SET status=?, published_at=?, updated_at=datetime('now') WHERE id=?`,
