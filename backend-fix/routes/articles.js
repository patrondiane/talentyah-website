...
/* IMPORTANT : placer les routes specifiques AVANT /:id
   sinon Express interprete "admin" comme un :id */

/* GET /api/articles/admin/all (admin) — tous les articles y compris non publies */
router.get('/admin/all', auth, (req, res) => {
  const rows = db.prepare('SELECT * FROM articles ORDER BY created_at DESC').all();
  res.json({ ok: true, total: rows.length, articles: rows });
});
...
router.post('/', auth, (req, res) => {
  ...
  const result = db.prepare(`
    INSERT INTO articles (title, category, excerpt, content, author, image_url, published)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(
    title.trim(), category || null, excerpt || null, content || null,
    author || 'Talentyah Editorial', image_url || null,
...
router.put('/:id', auth, (req, res) => {
  const { title, category, excerpt, content, author, image_url, published } = req.body;
  db.prepare(`
    UPDATE articles SET title=?, category=?, excerpt=?, content=?, author=?, image_url=?, published=?
    WHERE id=?
  `).run(
    title, category || null, excerpt || null, content || null,
    author || 'Talentyah Editorial', image_url || null,
    published ? 1 : 0,
    req.params.id
  );
