...
(() => {
  const getApiBase = () => {
    ...
    return isLocalHost ? "http://localhost:4001" : "https://talentyah-website.onrender.com";
  };
  ...
  function renderArticles(pubs) {
    ...
    grid.innerHTML = pubs.map(p => {
      ...
      const date = p.published_at
        ...
        : 'Talentyah Editorial';
      return `
        <article class="article-card reveal visible" data-category="${cat}" onclick="window.location.href='article.html?id=${p.id}'" role="button" tabindex="0" onkeydown="if(event.key==='Enter'||event.key===' ')window.location.href='article.html?id=${p.id}'">
